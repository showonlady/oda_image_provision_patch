...
"""
Usage:
    create_multiple_db_storch.py -h
    create_multiple_db_storch.py -s <servername> [-u <username>] [-p <password>]
...
def initlogger(hostname):
    global logfile
    logname = "create_multiple_databases_storch_%s.log" % hostname
    logfile = os.path.join(log_dir, logname)
    log = initlogging.initLogging("create_db_storch", logfile, logging.WARN, logging.DEBUG)
    return log
