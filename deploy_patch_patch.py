...
Usage:
    ...
    deploy_patch_patch.py [nopatch] -s <servername> [-u <username>] [-p <password>] [--pbd]
...
def provision_patch2(host, nopatchflag = False):
    if not host.is_dcs_or_oak():
        sys.exit(0)
    if not host.is_deployed_or_not():
        o_d.oda_deploy_new(host)
...
def main(arg):
    ...
    if arg['--pbd']:
        ...
        provision_patch2(host, nopatchflag)
