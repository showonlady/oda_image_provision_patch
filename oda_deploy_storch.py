...
Usage:
    oda_deploy.py -h
    oda_deploy.py  -s <servername> [-u <username>] [-p <password>] [--network <vlan>]
...
def oda_deploy_new(host, net_def):
    ...
    if flag:
        print "Successfully provision the system!"
...
def initlogger(hostname):
    global logfile
    logname = "odacli_deploy_storch_%s.log" % hostname
    logfile = os.path.join(log_dir, logname)
    log = initlogging.initLogging("oda_deploy_storch", logfile, logging.WARN, logging.DEBUG)
    return log
